...
-----------------------------------------------------------------------------
--
-- Check whether the dependency files need to be changed to the ones with the image graph 
require 'nn'
require 'torch'
require 'optim'
...
-- Add the parameters for the image_graph features such as image_graph_size etc and the maximum number of relations
...
-------------------------------------------------------------------------------
--Add the data loader for the image graph representation 
...
protos.img_graph_encoding=nn.img_graph_encoding(lmOpt)
...
local iparams, grad_iparams = protos.img_graph_encoding:getParameters()
...
print('total number of parameters in img_graph_encoding: ', iparams:nElement())
...
local function eval_split(split)
  ...
  protos.img_graph_encoding:evaluate()
  ...
  while true do
    ...
    if opt.gpuid >= 0 then
      ...
      data.img_graph=data.img_graph:cuda()
    end
  n = n + data.images:size(1)
  xlua.progress(n, total_num)
  
  local img_graph_feat=protos.img_graph_encoding:forward( { data.img_graph  }  )

  -- Check what all needs to be added here to accommodate the img_graph_feat
...
local function lossFun()
  ...
  -- Check what all needs to be modified in these files to accommodate img_graph_feat
  local word_feat, img_feat, w_ques, w_img, mask = unpack(protos.word:forward({data.questions, data.images}))
  ...
  -- Check the modification that needs to be done to allow to backpropagate through image_graph_encoding
  --
  --
  -----------------------------------------------------------------------------
  -- and lets get out!
