...
function attention.parallel_atten_fact(input_size_ques, input_size_img, embedding_size, ques_seq_size, img_seq_size , fact_size)
    local inputs = {}
    local outputs = {}
    ...
    -- Layer to add img_graph to the img_feat
    -- Layer to add img_graph to the ques_feat
    -- Rest Remains the same

    local ques_feat_orig = inputs[1]
    local img_feat_orig = inputs[2]
    local mask = inputs[3]
    local fact=inputs[4]

    local ques_fact=nn.JoinTable(2,2)( ques_feat_orig , fact  )
    local img_fact=nn.JoinTable(2,2)( img_feat_orig, fact) 

        
    local ques_feat=nn.Linear(input_size_ques+fact_size,input_size_ques)(ques_fact)
    local img_feat=nn.Linear(input_size_img + fact_size ,input_size_img)( img_fact  ) 
...
-- Will Add the image graph to it
...
function attention.alternating_atten(input_size_ques, input_size_img, embedding_size, ques_seq_size, img_seq_size)
    ...
    local img_feat = inputs[2]
    local mask = inputs[3]
    local img_graph=inputs[4]

    local ques_img_graph=nn.JoinTable(2,2)( ques_feat_orig , img_graph  )
    local img_img_graph=nn.JoinTable(2,2)( img_feat_orig, img_graph) 

        
    local ques_feat=nn.Linear(input_size_ques+image_graph_size,input_size_ques)(ques_img_graph)
    local img_feat=nn.Linear(input_size_img + image_graph_size ,input_size_img)( img_img_graph  ) 
...
function attention.recursive_atten(input_size, embedding_size, last_embed_size, output_size )
    ...
    local fact_hidden3=  nn.Tanh()(nn.Linear(last_embed_size + fact_size ,last_embed_size)(nn.JoinTable(2)({hidden3,fact}))) 
    local outfeat = nn.Linear(last_embed_size, output_size)(nn.Dropout(0.5)(fact_hidden3))
