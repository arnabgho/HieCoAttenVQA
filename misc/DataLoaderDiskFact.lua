...
function DataLoader:__init(opt)
    ...
    self.cap_train=h5_file:read('/cap_train'):all()
    self.cap_len_train=h5_file:read('/cap_len_train'):all()
    ...
    self.cap_test=h5_file:read('/cap_test'):all()
    self.cap_len_test=h5_file:read('/cap_len_test'):all()
...
function DataLoader:getBatch(opt)
    ...
    if split == 0 or split == 1 then
        ...
        data.captions=self.cap_train:index(1,ques_idx)
    else
        data.images = self.img_batch:view(batch_size, 196, -1):contiguous()
        ...
        data.captions=self.cap_test:index(1,ques_idx)
    end
    return data
end
